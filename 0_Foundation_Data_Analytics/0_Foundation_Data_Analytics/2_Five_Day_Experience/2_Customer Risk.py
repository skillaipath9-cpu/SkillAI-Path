...
# MAGIC FROM workspace.edufin_small.loans l;
...
# MAGIC SELECT 
# MAGIC   CASE 
# MAGIC     WHEN cibil_score < 600 THEN '<600'
# MAGIC     WHEN cibil_score >= 600 AND cibil_score <= 650 THEN '600-650'
# MAGIC     WHEN cibil_score >= 651 AND cibil_score <= 700 THEN '651-700'
# MAGIC     WHEN cibil_score >= 701 AND cibil_score <= 750 THEN '701-750'
# MAGIC     WHEN cibil_score > 750 THEN '750+'
# MAGIC   END AS cibil_segment,
# MAGIC   COUNT(loan_id) AS total_customers,
# MAGIC   COUNT(CASE WHEN loan_status = 'Defaulted' THEN 1 END) AS defaulted_count,
# MAGIC   ROUND((COUNT(CASE WHEN loan_status = 'Defaulted' THEN 1 END) * 100) / COUNT(*), 2) AS default_rate_pct
...
# MAGIC     WHEN age >= 36 AND age <= 45 THEN '36-45'
# MAGIC     WHEN age >= 46 AND age <= 60 THEN '46-60'
...
# MAGIC   ROUND(SUM(loan_amount) / 10000000, 2) AS total_exposure_cr
# MAGIC FROM workspace.edufin_small.loans
...
# MAGIC SELECT 
# MAGIC   CASE 
# MAGIC     WHEN annual_income < 300000 THEN '<3L'
# MAGIC     WHEN annual_income >= 300000 AND annual_income <= 600000 THEN '3L-6L'
# MAGIC     WHEN annual_income >= 600001 AND annual_income <= 1200000 THEN '6L-12L'
# MAGIC     WHEN annual_income > 1200000 THEN '12L+'
...
# MAGIC   ROUND(SUM(CASE WHEN loan_status = 'Defaulted' THEN loan_amount ELSE 0 END) / 100000, 2) AS defaulted_amount_cr
# MAGIC FROM workspace.edufin_small.loans
...
# MAGIC   ROUND((COUNT(CASE WHEN loan_status = 'Defaulted' THEN 1 END) * 100.0) / COUNT(*), 2) AS default_rate_pct
# MAGIC FROM workspace.edufin_small.loans
# MAGIC GROUP BY employment_type, income_segment
...
# MAGIC SELECT 
# MAGIC   customer_id,
# MAGIC   COUNT(CASE WHEN loan_status = 'Defaulted' THEN 1 END) AS default_count,
# MAGIC   SUM(CASE WHEN loan_status = 'Defaulted' THEN loan_amount ELSE 0 END) AS total_default_amount,
# MAGIC   MIN(cibil_score) AS min_cibil_score,
# MAGIC   (
# MAGIC     (COUNT(CASE WHEN loan_status = 'Defaulted' THEN 1 END) * 2) + 
# MAGIC     (SUM(CASE WHEN loan_status = 'Defaulted' THEN loan_amount ELSE 0 END) / 100000) + 
# MAGIC     (800 - MIN(cibil_score)) / 10
